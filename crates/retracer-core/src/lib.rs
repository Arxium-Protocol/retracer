...
impl Runner {
        ...
        B: ingestion::HasHeight + storage::IndexableBlock + Send + Sync + 'static,
    {
        ...
        // Webhook delivery is a subscriber like any SSE client, but one that
        // keeps its cursor in Postgres. Reads only, on the read pool.
        let dispatch = rest_service::webhooks::dispatch(
