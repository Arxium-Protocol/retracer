...
pub use corechain_payload::{ActionPayload, AssetRef, is_corechain_address};
...
fn raw_block_into_tolerant<P: serde::de::DeserializeOwned>(raw: RawBlock) -> Block<P> {
    let actions = raw
        ...
        .filter_map(|ra: RawAction| {
            match bincode::serde::decode_from_slice::<P, _>(
                &ra.payload,
                ...
                Err(err) => {
                    warn!(
                        sender = %ra.sender,
                        nonce = ra.nonce,
                        "skipping action with unrecognized payload: {err}"
                    );
                    None
                }
            }
    ...
    Block {
        ...
        round: raw.round,
        round_certificate: raw.round_certificate,
    }
}

fn decode_block_tolerant<P: serde::de::DeserializeOwned>(bytes: &[u8]) -> Result<Block<P>> {
    let raw: RawBlock = decode_exact(bytes)?;
    Ok(raw_block_into_tolerant(raw))
}

fn decode_sync_response_tolerant<P: serde::de::DeserializeOwned>(
    bytes: &[u8],
) -> Result<SyncResponse<Block<P>>> {
    // `SyncResponse<RawBlock>` shares `SyncResponse<Block<P>>`'s exact wire
    // layout — bincode encodes the enum tag and fields the same way regardless
    // of what `B` is — so this always decodes structurally, then each block's
    // actions get the same per-action tolerance as `decode_block_tolerant`.
    let raw: SyncResponse<RawBlock> = decode_exact(bytes)?;
    Ok(match raw {
        SyncResponse::Status { tip_height } => SyncResponse::Status { tip_height },
        SyncResponse::Blocks(blocks) => {
            SyncResponse::Blocks(blocks.into_iter().map(raw_block_into_tolerant).collect())
        }
...
/// Must match `arxd/network::gossip::blocks_topic` exactly. The node scopes
/// gossip by its genesis-hash-derived chain ID, preventing cross-chain blocks
/// from sharing one subscription.
...
/// message layout can no longer drift apart. `xc_wire::sync_protocol` is now
/// chain-scoped by the node's genesis-hash-derived `chain_id` (same reasoning
/// as the gossip topics), so there is no longer a single constant that
/// matches every chain — callers must supply the `chain_id` of the node they
/// intend to follow. Overridable outright via `Config::sync_protocol`
/// (`--sync-protocol`) for a chain that renames it.
pub fn default_sync_protocol(chain_id: &str) -> String {
    xc_wire::sync_protocol(chain_id)
}
...
/// Chain-owned verifier for a wire-v3 finality certificate. Returning an error
/// rejects the certificate; only `Ok(())` may advance the rollback floor.
pub type CertificateVerifier =
    Arc<dyn Fn(u64, Vec<u8>) -> futures::future::BoxFuture<'static, Result<()>> + Send + Sync>;
...
impl Config {
    ...
    pub fn for_chain(chain_id: &str) -> Self {
        Self {
            ...
            listen_port: 0,
            resume_from: None,
            blocks_topic: default_blocks_topic(chain_id),
...
impl<P: serde::de::DeserializeOwned + 'static> WireDecoder<Block<P>> {
    /// Decodes blocks structurally first (as [`RawBlock`]/[`RawAction`]),
    /// then decodes each action's payload as `P` — skipping instead of
    /// failing the block for any action whose payload variant `P` doesn't
    /// recognize. Use this for a chain where the local payload enum may lag
    /// the producing chain's; `exact()` remains the default everywhere else.
    pub fn tolerant() -> Self {
        Self::new(
            decode_block_tolerant::<P>,
            decode_sync_response_tolerant::<P>,
        )
    }
}
...
/// Every request is bound to both a peer and an expected response shape. A
/// response ID alone is not proof that a connected peer sent the response it
/// was asked for, especially when several requests are in flight.
#[derive(Clone, Debug)]
enum PendingRequest {
    Status {
        peer: PeerId,
...
impl PendingRequest {
    ...
    fn sent_at(&self) -> Instant {
        match self {
            Self::Status { sent_at, .. }
            | Self::NodeInfo { sent_at, .. }
            | Self::Blocks { sent_at, .. }
            | Self::Hashes { sent_at, .. }
            | Self::Certificate { sent_at, .. } => *sent_at,
        }
    }
}
...
fn send_sync_request(
    ...
    match bincode::serde::encode_to_vec(request, xc_primitives::wire_config()) {
...
fn track_sync_request(
    ...
    if let Some(request_id) = send_sync_request(swarm, peer, request) {
        ...
        let pending = match request {
            SyncRequest::Status => PendingRequest::Status {
                peer: *peer,
                sent_at,
            },
            SyncRequest::NodeInfo => PendingRequest::NodeInfo {
                peer: *peer,
                sent_at,
            },
            SyncRequest::Blocks { from } => PendingRequest::Blocks {
                peer: *peer,
                _from: *from,
                sent_at,
            },
            SyncRequest::Hashes { from, to } => PendingRequest::Hashes {
                peer: *peer,
                _from: *from,
                _to: *to,
                sent_at,
            },
            SyncRequest::Certificate { height } => PendingRequest::Certificate {
                peer: *peer,
                height: *height,
                sent_at,
            },
...
fn certificate_request_in_flight(
    pending_requests: &HashMap<request_response::OutboundRequestId, PendingRequest>,
    peer: &PeerId,
    height: u64,
) -> bool {
    pending_requests.values().any(|pending| {
        matches!(pending, PendingRequest::Certificate { peer: requested_peer, height: requested_height, .. } if requested_peer == peer && *requested_height == height)
    })
}
...
{
    run_with_decoder_and_certificate_verifier(
        config, block_tx, rewind_rx, network_tx, decoder, None,
    )
    .await
}

/// Equivalent to [`run_with_decoder`], with an optional chain-owned verifier
/// for finality certificates returned by wire-v3 peers.
        ...
        let event = tokio::select! {
            ...
            _ = status_poll.tick() => {
                pending_requests.retain(|_, pending| {
                    pending.sent_at().elapsed() <= STATUS_FRESHNESS_TIMEOUT
                });
            ...
            Some(from) = rewind_rx.recv() => {
                ...
                let peers: Vec<PeerId> = swarm.connected_peers().copied().collect();
                for peer in peers {
                    track_sync_request(&mut swarm, &peer, &SyncRequest::Blocks { from }, &mut pending_requests);
                }
                continue;
            }
        };
        match event {
            ...
            SwarmEvent::ConnectionClosed {
                ...
                if num_established == 0 {
                    ...
                    pending_requests.retain(|_, pending| pending.peer() != &peer_id);
                    publish_network_view(
                        &network_tx,
                        &active_peers,
                        &peer_tips,
            ...
            SwarmEvent::Behaviour(BehaviourEvent::Sync(request_response::Event::Message {
                ...
                let Some(pending_request) = pending_requests.remove(&request_id) else {
                    warn!(%peer, ?request_id, "ignoring unsolicited sync response");
                    continue;
                };
                if pending_request.peer() != &peer {
                    warn!(%peer, expected_peer = %pending_request.peer(), ?request_id, "ignoring sync response from unexpected peer");
                    continue;
                }
                if pending_request.sent_at().elapsed() > STATUS_FRESHNESS_TIMEOUT {
                    warn!(%peer, ?request_id, "ignoring expired sync response");
                    continue;
                }
                ...
                match sync_response {
                    SyncResponse::Status { tip_height } => {
                        if !matches!(&pending_request, PendingRequest::Status { .. }) {
                            warn!(%peer, ?request_id, "ignoring status response for a different request type");
                            continue;
                        }
                        reconcile_observed_tip(
                            peer,
                            tip_height,
                            Some(pending_request.sent_at()),
                            &mut observed_block_heights,
                        );
                    ...
                    SyncResponse::NodeInfo(info) => {
                        if !matches!(&pending_request, PendingRequest::NodeInfo { .. }) {
                            warn!(%peer, ?request_id, "ignoring node-info response for a different request type");
                            continue;
                        }
                        ...
                        status_seen_at.insert(peer, Instant::now());
                        // A peer's claimed finality height is not proof. Wire v3
                        // lets us fetch the certificate that must be independently
                        // verified before this map may advance.
                        if info.wire_version >= 3
                            && let Some(height) = info.finalized_height
                            && !certificate_request_in_flight(&pending_requests, &peer, height)
                        {
                            track_sync_request(
                                &mut swarm,
                                &peer,
                    ...
                    SyncResponse::Hashes(hashes) => {
                        if !matches!(&pending_request, PendingRequest::Hashes { .. }) {
                            warn!(%peer, ?request_id, "ignoring hash response for a different request type");
                            continue;
                        }
                    ...
                    SyncResponse::Blocks(mut blocks) => {
                        if !matches!(&pending_request, PendingRequest::Blocks { .. }) {
                            warn!(%peer, ?request_id, "ignoring block response for a different request type");
                            continue;
                        }
                    ...
                    SyncResponse::Certificate { height, record } => {
                        let PendingRequest::Certificate {
                            height: requested_height,
                            ..
                        } = &pending_request
                        else {
                            warn!(%peer, ?request_id, "ignoring certificate response for a different request type");
                            continue;
                        };
                        if height != *requested_height {
                            warn!(%peer, requested_height, received_height = height, "ignoring certificate response with mismatched height");
                            continue;
                        }
                        let Some(record) = record else {
                            info!(%peer, height, "peer has no finality certificate at claimed height");
                            continue;
                        };
                        let Some(verifier) = certificate_verifier.as_ref() else {
                            warn!(%peer, height, "certificate ignored: no chain verifier configured");
                            continue;
                        };
                        if let Err(err) = verifier(height, record).await {
                            warn!(%peer, height, "certificate verification failed: {err}");
                            continue;
                        }
                        finalized_by_peer.insert(peer, Some(height));
                        publish_network_view(
                            &network_tx,
                            &active_peers,
                            &peer_tips,
...
mod tests {
    ...
    /// with one action of every `ActionPayload` variant Arxium defines.
    /// Regenerate with `cargo run --manifest-path
    /// scripts/gen-v020-fixture/Cargo.toml` against a sibling Arxium
    /// checkout whenever Arxium adds/changes a variant — an appended
    /// variant doesn't move earlier discriminants, so only a fixture that
    /// actually contains the new variant catches the drift.
    #[test]
    fn decodes_arxium_v020_block_fixture() {
        // Captured from an Arxium v0.2.0 Block<ActionPayload>, encoded with
        // bincode's standard configuration under Arxium's length-prefixed
        // Action<P> wire format (Track B — see xc_primitives::RawAction).
        // Keep this independent of the active Rust type so schema drift
        // fails this compatibility check.
        //
        // ponytail: rev 5d433074f6447601de4adf45989061ae5551133f pinned in
        // Cargo.toml predates Track B — this fixture already reflects the
        // new wire format via the local `[patch]` override. Regenerate
        // (same command as above) and drop that patch once Track B is
        // committed/pushed and `rev` is updated to point at it.
        const V020_BLOCK_FIXTURE: &[u8] = &[
            ,
            ...
            ,
            ...
            ,
            ...
            0x3d, 0x30, 0x78, 0x66, 0x69, 0x78, 0x74, 0x75, 0x72, 0x65, 0x2d, 0x73, 0x74, 0x61,
            0x74, 0x65, 0x2d, 0x72, 0x6f, 0x6f, 0x74, 0x2d, 0x35, 0x64, 0x34, 0x33, 0x33, 0x30,
            0x37, 0x34, 0x66, 0x36, 0x34, 0x34, 0x37, 0x36, 0x30, 0x31, 0x64, 0x65, 0x34, 0x61,
            0x64, 0x66, 0x34, 0x35, 0x39, 0x38, 0x39, 0x30, 0x36, 0x31, 0x61, 0x65, 0x35, 0x35,
            0x35, 0x31, 0x31, 0x33, 0x33, 0x66, 0x00, 0x00,
        ];
        ...
        assert_eq!(
            block.state_root,
            "0xfixture-state-root-5d433074f6447601de4adf45989061ae5551133f"
        );
        assert_eq!(block.tx_root, [7; 32]);
        assert_eq!(block.round, 0);
        assert!(block.round_certificate.is_none());

        // The actual gate: one action per known variant, in discriminant
        // order. A variant Arxium adds after `SubmitExecutionFault` doesn't
        // shift these indices (bincode encodes by discriminant), so this
        // only catches drift because the fixture is regenerated — not
        // because the count would otherwise change.
        assert_eq!(
            block.actions.len(),
            28,
            "one action per ActionPayload variant"
        );
        let kinds: Vec<&'static str> = block
            ...
            .map(|a| match &a.payload {
                ...
                ActionPayload::JoinValidator { .. } => "JoinValidator",
                ActionPayload::LeaveValidator { .. } => "LeaveValidator",
                ActionPayload::Stake { .. } => "Stake",
                ActionPayload::Unstake { .. } => "Unstake",
                ActionPayload::SubmitEquivocationEvidence { .. } => "SubmitEquivocationEvidence",
                ActionPayload::RegisterBlsKey { .. } => "RegisterBlsKey",
                ActionPayload::VerifyIdentityCredential { .. } => "VerifyIdentityCredential",
                ActionPayload::AuthorizeOperator { .. } => "AuthorizeOperator",
                ActionPayload::RevokeOperator => "RevokeOperator",
                ActionPayload::GrantAttestation { .. } => "GrantAttestation",
                ActionPayload::RevokeAttestation { .. } => "RevokeAttestation",
                ActionPayload::RegisterAsset { .. } => "RegisterAsset",
                ActionPayload::IssueAsset { .. } => "IssueAsset",
                ActionPayload::TransferAsset { .. } => "TransferAsset",
                ActionPayload::RegisterAttestor { .. } => "RegisterAttestor",
                ActionPayload::DeregisterAttestor { .. } => "DeregisterAttestor",
                ActionPayload::SubmitExecutionFault { .. } => "SubmitExecutionFault",
        ...
        match &block.actions[12].payload {
            ActionPayload::RegisterAsset {
                asset_id, metadata, ..
            } => {
                assert_eq!(asset_id, "a");
                assert_eq!(metadata.symbol, "AAA");
                assert_eq!(metadata.name, "Asset A");
            }
            other => panic!("index 12 should be RegisterAsset, got {other:?}"),
        }
    }

    /// Pins the discriminant of the last known variant. On its own this
    /// stays green forever — an appended Arxium variant never moves
    /// `SubmitExecutionFault`'s index — so it only means something paired
    /// with `decodes_arxium_v020_block_fixture` actually containing that
    /// variant.
    #[test]
    fn submit_execution_fault_is_discriminant_17() {
        let payload = ActionPayload::SubmitExecutionFault {
            artifact_json: "{}".to_string(),
        };
        let encoded = bincode::serde::encode_to_vec(&payload, bincode::config::standard())
            .expect("payload encoding should never fail");
        assert_eq!(encoded[0], 17);
    }

    #[test]
    fn pre_reset_two_field_register_asset_is_not_accepted_as_current_wire() {
        // Runtime changed this in place only with a devnet genesis reset. Do
        // not silently decode the old signed bytes as a current action: that
        // would make the action signature and transaction root unverifiable.
        #[allow(dead_code)] // Placeholder variants preserve RegisterAsset's wire index.
        #[derive(serde::Serialize)]
        enum PreResetPayload {
            Transfer,
            JoinValidator,
            LeaveValidator,
            Stake,
            ...
            GrantAttestation,
            RevokeAttestation,
            RegisterAsset { asset_id: String, compliance_required: bool },
        }

        let bytes = bincode::serde::encode_to_vec(
            PreResetPayload::RegisterAsset { asset_id: "legacy".into(), compliance_required: false },
            xc_primitives::wire_config(),
        )
        .unwrap();
        assert!(decode_exact::<ActionPayload>(&bytes).is_err());
    }
    ...
    #[test]
    fn chain_defaults_use_the_genesis_scoped_node_topics() {
        let chain_id = "a1b2c3";
        let config = Config::for_chain(chain_id);
        assert_eq!(config.blocks_topic, "arxium/blocks/v1/a1b2c3");
        assert_eq!(config.sync_protocol, xc_wire::sync_protocol(chain_id));
    }
    ...
    /// Track B's length-prefixed wire format.
    #[test]
    fn tolerant_decoder_skips_unknown_action_variant_instead_of_failing_the_block() {
        let sender = xc_primitives::Address::from_pubkey_bytes(&[7u8; 32]).unwrap();
        let known_bytes = bincode::serde::encode_to_vec(
        ...
        let raw = RawBlock {
            ...
            actions: vec![
                RawAction {
                    ...
                    nonce: 1,
                    signature: None,
                    payload: vec![99, 0xaa, 0xbb], // unknown discriminant, some payload bytes
                },
                RawAction {
                    sender,
                    nonce: 2,
        ...
        assert!(
            decode_exact::<Block<ActionPayload>>(&encoded).is_err(),
            "exact decode should still fail on an unrecognized variant"
        );

        let block: Block<ActionPayload> =
            decode_block_tolerant(&encoded).expect("tolerant decode should skip, not fail");
        assert_eq!(block.actions.len(), 1, "unknown-variant action is dropped");
        assert_eq!(block.actions[0].nonce, 2);
        assert!(matches!(
    ...
    fn tolerant_decoder_skips_non_canonically_padded_payload() {
        ...
        assert!(
            ...
            "padded action must be dropped, not accepted"
        );
    }
