...
# Per-IP requests/second. Blank disables rate limiting.
RETRACER_RATE_LIMIT_RPS=

# Interface the API listens on. 127.0.0.1 keeps it loopback-only; set to
# 0.0.0.0 only behind this host's firewall or a private/WireGuard network,
# and only alongside RETRACER_AUTH_TOKEN above — HTTP is plaintext.
